...
real(b8) function Wmin( k, theta)
    implicit none
    integer,  intent(in) :: k
    real(b8), intent(in), dimension(:) :: theta
...
! update thetaillator
real(b8) function getOmega( k, ntrlOmega, theta)
    implicit none
    integer,  intent(in) :: k
    real(b8), intent(in) :: ntrlOmega
    real(b8), intent(in), dimension(:) :: theta

    getOmega = ntrlOmega + Wmin( k, theta) + normal( 0.0000_b8, sqrt(dt))
